...
class JobMonitor:
    ...
    def get_condor_status(self, cluster_id: str) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                ['condor_q', cluster_id, '-format', '%s', 'JobStatus'],
                capture_output=True,
                text=True,
                timeout=5
            )
    ...
    def print_detailed_status(self, summary: Dict, filter_status: str = None):
        ...
        for job in jobs:
            ...
            print(f"   Parameters: n_networks={job['param_set']['n_networks']}, "
    ...
    def check_failed_jobs(self, summary: Dict) -> List[Dict]:
        ...
        for job in summary['jobs']:
            ...
            if (job['condor_status'] == 'completed' and 
                not job['benchmark_complete']):
                failed.append(job)
            # Or if job is held
...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor PseudoModel benchmark job progress"
    )
    parser.add_argument(
        '--jobs-dir',
        type=str,
    ...
    parser.add_argument(
        '--detailed',
        action='store_true',
        help='Show detailed job-by-job status'
    )
    parser.add_argument(
        '--filter-status',
        type=str,
    ...
    parser.add_argument(
        '--check-failed',
        action='store_true',
        help='Only show potentially failed jobs'
    )
