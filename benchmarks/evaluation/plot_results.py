...
class BenchmarkAnalyzer:
    ...
    def plot_3d_scatter(self, strategy: str = "sequential", output_path: str = None):
        ...
        ax.set_title(f"PseudoModel Performance - {strategy.upper()} Strategy", fontsize=14, fontweight="bold", pad=20)
    ...
    def plot_2d_projections(self, strategy: str = "sequential", output_path: str = None):
        ...
        fig.suptitle(f"2D Projections - {strategy.upper()} Strategy", fontsize=14, fontweight="bold", y=1.02)
    ...
    def plot_1d_slices(self, strategy: str = "sequential", output_path: str = None):
        ...
        for ax, (dim_col, dim_label) in zip(axes, dimensions):
            ...
            ax.fill_between(x_vals, grouped["min"].values, grouped["max"].values, alpha=0.2, label="Min-Max Range")
        ...
        fig.suptitle(
            f"1D Behavior Along Each Dimension - {strategy.upper()} Strategy", fontsize=14, fontweight="bold", y=1.02
        )
    ...
    def plot_strategy_comparison(self, output_path: str = None):
        ...
        for strategy in self.strategies:
            ...
            if len(subset) > 0:
                ...
                ax.plot(grouped.index, grouped.values, "o-", label=strategy.upper(), linewidth=2, markersize=8)
        ...
        for strategy in self.strategies:
            ...
            if len(subset) > 0:
                ...
                ax.bar(
                    [p + 0.25 * self.strategies.index(strategy) for p in x_positions],
                    grouped.values,
                    width=0.25,
        ...
        fig.suptitle("Strategy Comparison Across All Dimensions", fontsize=16, fontweight="bold", y=1.00)
    ...
    def generate_summary_statistics(self, output_path: str = None):
        ...
        summary = (
            self.data.groupby("strategy")["time_ms"]
            .agg(
                [
                    "count",
                    "mean",
    ...
    def create_all_plots(self, output_dir: str):
        ...
        self.generate_summary_statistics(output_path / "summary_statistics.csv")
        ...
        for strategy in self.strategies:
            ...
            # 3D plot
            self.plot_3d_scatter(strategy=strategy, output_path=output_path / f"{strategy}_3d.png")
            plt.close()

            # 2D projections
            self.plot_2d_projections(strategy=strategy, output_path=output_path / f"{strategy}_2d_projections.png")
            plt.close()

            # 1D slices
            self.plot_1d_slices(strategy=strategy, output_path=output_path / f"{strategy}_1d_slices.png")
            plt.close()
...
def main():
    parser = argparse.ArgumentParser(description="Analyze and visualize PseudoModel benchmark results")
    parser.add_argument("--results-dir", type=str, required=True, help="Directory containing benchmark JSON files")
    ...
    if args.plot_type == "all":
        ...
        analyzer.plot_3d_scatter(strategy=args.strategy, output_path=Path(args.output_dir) / f"{args.strategy}_3d.png")
    elif args.plot_type == "2d":
        analyzer.plot_2d_projections(
            strategy=args.strategy, output_path=Path(args.output_dir) / f"{args.strategy}_2d.png"
        )
    elif args.plot_type == "1d":
        analyzer.plot_1d_slices(strategy=args.strategy, output_path=Path(args.output_dir) / f"{args.strategy}_1d.png")
    elif args.plot_type == "comparison":
        analyzer.plot_strategy_comparison(output_path=args.output_dir)
    elif args.plot_type == "stats":
        analyzer.generate_summary_statistics(output_path=Path(args.output_dir) / "summary_statistics.csv")
