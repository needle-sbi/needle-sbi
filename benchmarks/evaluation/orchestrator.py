...
class GridSearchOrchestrator:
    ...
    def generate_parameter_grid(self) -> List[Dict[str, Any]]:
        ...
        for ensemble, fold, batch_size, net_config in itertools.product(
            ...
            n_params = self.calculate_n_params(net_config["hidden_dim"], net_config["n_hidden"])
    ...
    def create_config_files(self, param_set: Dict[str, Any], job_dir: Path) -> Dict[str, Path]:
        ...
        for estimator_name in main_config["estimators"]:
            main_config["estimators"][estimator_name]["expands"]["ensembles"]["num_ensembles"] = param_set[
                "num_ensembles"
            ]
    ...
    def create_htcondor_submission(self, param_set: Dict[str, Any], job_dir: Path, job_id: int) -> Tuple[Path, Path]:
        """
...
"""
    ...
    def submit_job(self, sub_file_path: Path, dry_run: bool = False) -> str:
        ...
        result = subprocess.run(["condor_submit", str(sub_file_path)], capture_output=True, text=True)
    ...
    def run_grid_search(self, dry_run: bool = False, max_jobs: int = None):
        ...
        for job_id, param_set in enumerate(param_grid):
            ...
            print(
                f"  n_networks={param_set['n_networks']}, "
            ...
            sub_file, run_script = self.create_htcondor_submission(param_set, job_dir, job_id)
    ...
    def run_grid_search_with_law_remote(self, dry_run: bool = False, max_jobs: int = None):
        ...
        for job_id, param_set in enumerate(param_grid):
            ...
            law_cmd = [
                ...
                "MainTask",
                "--config-file",
                str(config_paths["main"]),
                "--SnapshotTask-rel-results-path",
                output_dir,
                # Enable HTCondor workflow at all levels
                "--MainTask-workflow",
                "htcondor",
                "--SnapshotTask-workflow",
                "htcondor",
                "--EstimatorTask-workflow",
                "htcondor",
                "--SystematicTask-workflow",
                "htcondor",
                "--EnsembleTask-workflow",
                "htcondor",
                "--FoldTask-workflow",
                "htcondor",  # Pass through for consistency
                # HTCondor resource requests for workflow tasks
                "--EnsembleTask-htcondor-request-cpus",
                "2",
                "--EnsembleTask-htcondor-request-memory",
                "16000",
                "--SystematicTask-htcondor-request-cpus",
                "2",
                "--SystematicTask-htcondor-request-memory",
                "16000",
                "--EstimatorTask-htcondor-request-cpus",
                "2",
                "--EstimatorTask-htcondor-request-memory",
                "16000",
                # Workflow settings
                "--poll-interval",
                "30",
                "--parallel-jobs",
                "50",  # Submit up to 50 jobs concurrently
                "--log-level",
                "INFO",
            ]
    ...
    def _run_benchmark(self, snapshot_path: Path, param_set: Dict, job_id: int):
        ...
        benchmark_cmd = [
            ...
            str(snapshot_path),
            "--output",
            str(self.results_dir / f"benchmark_job_{job_id}.json"),
            "--n-tests",
            str(self.grid_config["benchmark"]["n_tests"]),
            "--warmup",
            str(self.grid_config["benchmark"]["warmup_iterations"]),
            "--test-size",
            str(self.grid_config["benchmark"]["test_data_size"]),
            "--param-set",
...
def main():
    parser = argparse.ArgumentParser(description="Orchestrate grid search for PseudoModel benchmarking")
    parser.add_argument(
        "--grid-config", type=str, default="benchmarks/grid_config.yaml", help="Path to grid configuration file"
    )
    ...
    parser.add_argument("--max-jobs", type=int, default=None, help="Maximum number of jobs to submit (for testing)")
    parser.add_argument(
        "--mode",
        type=str,
    ...
    # Create orchestrator
    orchestrator = GridSearchOrchestrator(grid_config_path=args.grid_config, workspace_root=args.workspace)
    ...
    if args.mode == "law-remote":
        ...
        orchestrator.run_grid_search_with_law_remote(dry_run=args.dry_run, max_jobs=args.max_jobs)
    else:
        print("Using wrapper script mode")
        print("  - Submits wrapper HTCondor jobs")
        print("  - Each job runs entire DAG locally\n")
        orchestrator.run_grid_search(dry_run=args.dry_run, max_jobs=args.max_jobs)
