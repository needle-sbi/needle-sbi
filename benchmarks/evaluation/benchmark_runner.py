...
def profile_sequential(
    model: PseudoModel, test_data: torch.Tensor, warmup: int = 3, n_tests: int = 10
) -> Dict[str, float]:
    """
    ...
    avg_timings = {key: np.mean([t[key] for t in timings]) for key in timings[0].keys()}

    return avg_timings


def profile_parallel(
    model: PseudoModelParallel, test_data: torch.Tensor, warmup: int = 3, n_tests: int = 10
) -> Dict[str, float]:
    """
...
def profile_vectorized(
    model: PseudoModelVectorized, test_data: torch.Tensor, warmup: int = 3, n_tests: int = 10
) -> Dict[str, float]:
    """
...
def run_benchmarks(
    snapshot_path: str, test_data: torch.Tensor, warmup: int = 3, n_tests: int = 10, param_set: Dict[str, Any] = None
...
def main():
    parser = argparse.ArgumentParser(description="Run PseudoModel benchmarks and save results")
    parser.add_argument("--snapshot", type=str, required=True, help="Path to dag_snapshot.json")
    parser.add_argument("--output", type=str, required=True, help="Output JSON file for results")
    parser.add_argument("--n-tests", type=int, default=10, help="Number of test iterations")
    parser.add_argument("--warmup", type=int, default=3, help="Number of warmup iterations")
    parser.add_argument("--test-size", type=int, default=1000, help="Number of samples in test data")
    parser.add_argument("--param-set", type=str, default=None, help="JSON string of parameter set configuration")
    ...
    results = run_benchmarks(
        snapshot_path=args.snapshot, test_data=test_data, warmup=args.warmup, n_tests=args.n_tests, param_set=param_set
    )
